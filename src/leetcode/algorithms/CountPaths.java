...
public class CountPaths {
    ...
    public static int countPaths(int n, int[][] roads) {
        ...
        while (!queue.isEmpty()) {
             ...
             */
            for (Road road : graphs[start]) {
                int to = road.to;
                long textTime = times[start] + road.time;

                if (textTime < times[to]) {
                    /**
                     * 得到一种用时更少的从路口0到达路口to的方案，之前经过路口to的方案要根据当前最优方案重新计算
                     */
                    times[to] = textTime;
                    queue.offer(to);
                }
        ...
        /**
         * 查找可以用时times[n-1]从路口0到达路口n-1的方案数
         */
        dfs(graphs, times, n - 1, times[n - 1], -1, 0, 0L);
        return result;
    }
     ...
     * @param destination
     * @param minTime
     * @param from        从路口from来到路口current
     * @param current     当前所在路口
     * @param time        到达当前路口总用时
     */
    public static void dfs(List<Road>[] graphs, long[] times, int destination, long minTime, int from, int current, long time) {
        if (current == destination && time == minTime) {
            result = (result + 1) % mod;
            return;
        }
        ...
        for (Road road : graphs[current]) {
            ...
            dfs(graphs, times, destination, minTime, current, road.to, time + road.time);
        }
    }
