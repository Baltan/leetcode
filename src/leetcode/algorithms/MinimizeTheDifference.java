...
public class MinimizeTheDifference {
     ...
     * @param target
     */
    public static void dfs(int[][] mat, int rows, int cols, int row, int sum, int target) {
        if (row == rows) {
        ...
        for (int col = 0; col < cols; col++) {
            /**
             * 如果当前已选择的数字之和sum减去target已经不小于result了，不需要再继续递归选择其他数字，因为选择其他数字后绝对差只会更大
             */
            if (sum + mat[row][col] - target >= result) {
                continue;
            }
            dfs(mat, rows, cols, row + 1, sum + mat[row][col], target);
        }
    }
}
